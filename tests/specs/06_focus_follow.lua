...
H.check("code view raised by blur", H.zindex(cw) == H.Z.top, "z=" .. tostring(H.zindex(cw)))
H.check("cli lowered by blur", H.zindex(H.term_win()) == H.Z.hidden, "z=" .. tostring(H.zindex(H.term_win())))

require("sidekick.cli").focus() -- not focused, so this focuses
vim.wait(500)
H.check("cli raised by focus", H.zindex(H.term_win()) == H.Z.top)
H.check("cursor on the cli", H.cur() == H.term_win())

-- sending context must bring the CLI forward too
H.key(Z.config.keys.code)()
vim.wait(400)
H.check("back on the code view", H.zindex(cw) == H.Z.top)
require("sidekick.cli").send({ msg = "{file}" })
vim.wait(1200)
H.check("send raised the cli view", H.zindex(H.term_win()) == H.Z.top, "z=" .. tostring(H.zindex(H.term_win())))
H.check("send lowered the code view", H.zindex(cw) == H.Z.hidden, "z=" .. tostring(H.zindex(cw)))
